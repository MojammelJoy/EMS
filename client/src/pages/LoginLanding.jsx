       ...
       <div className="w-full max-w-md animate-fade-in relative z-10">
           ...
           <div className="space-y-4">
            {portalOptions.map((portal)=>(
              <Link key={portal.to} to={portal.to} className="group block bg-slate-50 border border-slate-200 rounded-lg p-5 sm:p-6 transition-all duration-300 hover:border-orange-500 hover:bg-orange-50">
                <div className="relative z-10 flex items-center justify-between gap-4 sm:gap-5">
                  <h3 className="text-lg text-slate-800 group-hover:text-orange-600 mb-1 transition-colors">{portal.title}</h3>
           ...
           <div className="mt-12 text-center md:text-left text-sm text-slate-500">
              <p> © {new Date().getFullYear()} Stackleo. All rights reserved </p>
           </div>
       </div>
      </div>
    </div>
